...
const issueDetails: IssueDetail[] = [
  ...
  },
  {
    ...
    title: "Перед передачей проекта я упростил доступ",
    summary: "Для тестового задания оказалось важнее убрать лишние барьеры, чем удерживать демо в production-режиме.",
    problems: [
      "Проверяющему нужно было отдельно передавать логин, пароль и секрет выгрузки.",
      "Из-за этого часть сценариев нельзя было пройти с первого открытия приложения.",
      "Для тестовых mock data такая защита только усложняла вход.",
    ],
    solution: [
      "Убрал Basic Auth с dashboard.",
      "Убрал обязательный x-pipeline-secret из import route и UI.",
      "Снял secret-проверки с webhook routes, чтобы не дублировать настройку в Vercel и Supabase.",
    ],
    impact: "Теперь проект открывается и проверяется сразу: можно зайти в dashboard, загрузить mock JSON и добавить заказ вручную без отдельного набора секретов.",
    accent: "#8b92ff",
